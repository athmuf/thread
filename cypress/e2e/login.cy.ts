...
describe('Login spec', () => {
  ...
  it('should display alert when password is empty', () => {
    cy.contains('button', 'Log In').click()
    cy.get('input[placeholder="johndoe@example.com"]').type('testuser@example.com');
  ...
  it('should display alert when username and password are wrong', () => {
    cy.contains('button', 'Log In').click()
    cy.get('input[placeholder="johndoe@example.com"]').type('testuser@yopmail.com');
  ...
  it('should display homepage when username and password are correct', () => {
    cy.contains('button', 'Log In').click()
    cy.get('input[placeholder="johndoe@example.com"]').type('testfe2@yopmail.com');
    cy.get('input[type="password"]').type('123456');
    cy.get('button').contains(/^Login$/).click();
    cy.get('div[data-slot="dropdown-menu-trigger"]').click()
    cy.get('div[data-slot="dropdown-menu-label"]').contains('Log Out').should('be.visible');
  });
});